...

{
  ...
  $mdp=$_POST['mdp'];
  	...
  	if($nom!="" && $prenom!="" && $genre!="" && $type!="" && $niveau_etudes!="" && $email!="" && $mdp!="")
  {  
     
     $sql="UPDATE `inscription` SET". "`nom` ='$nom',"."`prenom` = '$prenom',"."`genre` = '$genre',"."`type` = '$type',". "`niveau_etudes` = '$niveau_etudes',"."`mdp` = '$mdp'". " WHERE `inscription`.`email` ="."'$email'";  
...
footer
{
 ...
 input[type="submit"]{
  ...
  <td>Mot de passe</td>
   <td><input type="password" name="mdp" placeholder="mdp"/></td>
  </tr>
    <tr>
